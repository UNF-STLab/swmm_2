...
//  Data Structures
//-----------------------------------------------------------------------------
// OWA EDIT - LidList and LidGroup struct defs moved to lid.h to be shared by toolkit.c
...
//
{
    ...
    // to allow multiple calls to validateLidProcs when setting Lid params in toolkit
...
//
{
    ...
    for (j = 0; j < GroupCount; j++)
    {
        ...
        while ( lidList )
        {
            ...
            // OWA EDIT in 86bccdffbee0d07716b71044e2890deac1529fd3
            // initialize water rate add more output to lid api toolkit 
            // to show detailed report values / lid layer rates
            // lidproc_initWaterRate not defined in EPA SWMM
...
// OWA EDIT ########################################################
// additional lid getter and setter functions for toolkit api
...
// Purpose: Gets lid unit (TLidUnit) ptr
{
    ...
    if (!lidGroup)
    {
        *errcode = ERR_TKAPI_UNDEFINED_LID;
    }
    else
    {
        lidList = lidGroup->lidList;
        ...
        {
            *errcode = ERR_TKAPI_LIDUNIT_INDEX;
            return(NULL);
        }
        else
        {
